...
1. 自动备份当前数据
2. 下载最新版本并验证 SHA256
3. 停止服务（指纹捕获与替换都在停服窗口内完成；新版本的指纹捕获会顺带完成数据库迁移，停服先行可避免迁移与运行中的旧服务并发写库）
4. 替换二进制文件
5. 验证状态指纹（确保数据未丢失）
6. 重启服务
7. 验证健康检查端点

如果更新失败，自动回滚到旧版本：同时还原旧二进制与升级前备份的状态目录（数据库一并回到升级前状态，已执行的迁移随之撤销）。回滚消息会给出备份归档路径，状态还原失败时请按提示从该归档手工恢复。
