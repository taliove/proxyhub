...
**订阅地址不含 Site Path**（2026-08 起，issue #74）：订阅端点挂在根命名空间 `/sub/{path}?token=...`，订阅链接不再携带 Site Path——链接随客户端配置/分享/日志扩散是常态，不该顺带泄露管理面入口。端点本身有随机 path + token，公开可访问但不可枚举。历史已发出的 `/<SitePath>/sub/...` 旧链接继续可用（双挂兼容），新复制的链接一律为根命名空间形式。
...
- [ ] **重置泄露的订阅链接**（订阅地址被分享/泄露被蹭用时：用户个人中心对该订阅地址执行「重置链接」，或管理端用户管理「订阅链接」代为重置。旧链接默认 3 天宽限、可每次 +3 天延长，给在用设备留迁移窗口；宽限期后旧链接一律 404，蹭用者失去访问能力。宽限期内的蹭用拉取在拉取统计里以「宽限下发」状态单独可见，据此确认蹭用是否消失）
