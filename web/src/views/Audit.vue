<template>
  <div>
      ...
      <template #header>
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 8px;">
            <el-select v-model="filterTypes" multiple collapse-tags placeholder="事件类型" style="width: 220px;" @change="reload">
            ...
            <el-input v-model="filterIP" placeholder="按 IP 搜索" style="width: 160px;" clearable @change="reload" />
      ...
      <el-table :data="events" v-loading="loading">
        ...
        <el-table-column label="事件" width="120">
          <template #default="{ row }">
            <el-tag :type="eventTag(row.event_type)" size="small">{{ eventLabel(row.event_type) }}</el-tag>
          </template>
        </el-table-column>
    ...
    <el-card style="margin-top: 20px;">
      ...
      <el-table :data="banned" v-loading="bannedLoading">
      ...
      <el-empty v-if="!bannedLoading && banned.length === 0" description="暂无封禁记录" :image-size="60" />
    </el-card>
  </div>
</template>
...
const load = async () => {
  ...
  try {
    ...
    const data = await client.get<any, { events: AuditEvent[]; total: number }>(`/audit/events?${params}`)
