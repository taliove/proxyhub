<template>
  <div>
    <el-card v-if="activeRun" style="margin-bottom: 16px;">
      <template #header>
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <el-button v-if="activeRun.status !== 'running'" link type="primary" @click="activeRun = null">收起</el-button>
        </div>
      </template>
      <el-alert v-if="pollError" type="warning" :closable="false" show-icon style="margin-bottom: 8px;">
        轮询中断：{{ pollError }}，请稍后手动刷新列表
      </el-alert>
      <div>
        <div v-for="ev in activeEvents" :key="ev.id" style="display: flex; gap: 8px; padding: 4px 0; align-items: flex-start;">
          <el-icon style="margin-top: 2px;"><component :is="levelIcon(ev.level)" /></el-icon>
          <span style="color: #909399; font-size: 12px; min-width: 70px;">{{ formatTime(ev.created_at) }}</span>
    ...
    <el-card>
      ...
      <el-table :data="runs" v-loading="loading" row-key="id" @expand-change="onExpand">
        <el-table-column type="expand">
          <template #default="{ row }">
            <div style="padding: 12px 24px;">
              ...
              <div v-else>
                <div v-for="group in groupedEvents(row.id)" :key="group.stage" style="margin-bottom: 16px;">
                  <div style="font-weight: 600; margin-bottom: 8px; color: #606266;">
                    {{ stageLabel(group.stage) }}
                  </div>
                  <div v-for="ev in group.events" :key="ev.id" style="display: flex; gap: 8px; padding: 4px 0; align-items: flex-start;">
                    <el-icon style="margin-top: 2px;"><component :is="levelIcon(ev.level)" /></el-icon>
                    <span style="color: #909399; font-size: 12px; min-width: 70px;">{{ formatTime(ev.created_at) }}</span>
                    <span style="flex: 1;">{{ ev.message }}</span>
                  ...
                  <pre v-if="openDataShow(group.events)" style="background: #f5f7fa; padding: 8px; margin: 4px 0 0 84px; font-size: 12px; overflow-x: auto;">{{ prettyData(group.events) }}</pre>
                </div>
              </div>
            </div>
          </template>
...
const loadEvents = async (runId: number) => {
  try {
    const resp = await client.get(`/refresh/runs/${runId}`) as { run: RefreshRun; events: RefreshEvent[] }
...
interface EventGroup { stage: string; events: RefreshEvent[] }
...
const startPolling = (runId: number) => {
  ...
  pollError.value = ''
  pollRetries = 0
  activeRun.value = { id: runId, status: 'running', trigger: 'manual', total_nodes: 0, available_nodes: 0, final_nodes: 0, error: '', started_at: new Date().toISOString(), finished_at: null }
  activeEvents.value = []
  pollOnce(runId, gen)
}
...
const statusTagType = (s: string): 'success' | 'warning' | 'danger' | 'primary' | 'info' =>
  ({ success: 'success', partial: 'warning', failed: 'danger', running: 'primary' } as const)[s as 'success'] || 'info'

const statusLabel = (s: string): string =>
  ({ success: '成功', partial: '部分', failed: '失败', running: '进行中' } as Record<string, string>)[s] || s

const stageLabel = (s: string): string =>
  ({ fetch: '拉取', check: '健康检查', filter: '过滤', done: '完成' } as Record<string, string>)[s] || s

const levelIcon = (l: string) =>
  ({ info: InfoFilled, warn: WarningFilled, error: CircleCloseFilled } as Record<string, any>)[l] || InfoFilled
