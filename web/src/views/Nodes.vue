<template>
  <el-card>
    <template #header>
      <div class="header">
        <span>节点管理</span>
        <span v-if="activeTab === 'airport' && lastUpdate" class="muted"
          >最后更新：{{ formatTime(lastUpdate) }}</span
        >
      </div>
    </template>
    ...
    <el-tabs v-model="activeTab" @tab-change="handleTabChange">
      <el-tab-pane label="机场节点" name="airport">
        ...
        <div class="batch-bar">
          ...
          <el-button type="danger" size="small" :disabled="!bulkSource" @click="blockBySource"
            >屏蔽该机场</el-button
          >
          <el-button type="warning" size="small" :disabled="!bulkSource" @click="unblockBySource"
            >取消该机场</el-button
          >
        ...
        >
          ...
          <el-table-column type="expand">
            <template #default="{ row }">
              ...
              <div v-else class="muted" style="padding: 8px 16px">
                该节点暂无检测记录,点上方「检测」按钮运行检测。
              </div>

              <!-- 带宽测试结果块 -->
              <div v-if="row.bandwidth_down_mbps || row.bandwidth_up_mbps" class="bw-detail">
                <span class="bw-label">带宽测试:</span>
                <el-tag size="small" type="success"
                  >下行 {{ (row.bandwidth_down_mbps || 0).toFixed(1) }} Mbps</el-tag
                >
                <el-tag size="small" type="success"
                  >上行 {{ (row.bandwidth_up_mbps || 0).toFixed(1) }} Mbps</el-tag
                >
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="180">
            <template #default="{ row }">
              ...
              <template v-else>
                ...
                <el-tag v-if="row.blocked" type="warning" size="small" style="margin-right: 6px"
                  >已屏蔽</el-tag
                >
                <el-button v-if="row.blocked" link type="warning" @click="unblockNode(row)"
                  >取消屏蔽</el-button
                >
          ...
          <el-table-column label="测试" width="200" fixed="right">
            <template #default="{ row }">
              <el-button-group size="small">
                ...
                <el-button type="success" :disabled="testing" @click="runTest(row, 'bandwidth')"
                  >带宽</el-button
                >
              </el-button-group>
            </template>
        ...
        <el-dialog v-model="override.visible" title="编辑节点覆盖" width="420px">
          ...
          <template #footer>
            ...
            <el-button type="primary" :loading="override.saving" @click="saveOverride"
              >保存</el-button
            >
          </template>
        </el-dialog>
        ...
        <el-dialog v-model="cleanup.visible" title="清理失败节点" width="480px">
          ...
          <template v-else>
            <el-alert type="warning" :closable="false" style="margin-bottom: 12px">
              <template #title>
                将处理 {{ cleanup.failedNodes.length }} 个不可用节点:机场
                {{ cleanupAirportCount }} 个、自建 {{ cleanupSelfCount }} 个
              </template>
            </el-alert>
...
const cleanupSelfCount = computed(() => cleanup.failedNodes.filter((n) => isSelfHosted(n)).length)
