...
// getRefreshRun 拉取单条刷新记录及事件流。
export function getRefreshRun(id: number): Promise<{ run: RefreshRun; events: RefreshEvent[] }> {
  return client.get<unknown, { run: RefreshRun; events: RefreshEvent[] }>(`/refresh/runs/${id}`)
}
