...
func (a *Aggregator) PurgeAirportNodes() (int, error) {
	...
	removed, err := a.st.DeleteAirportNodes()
	if err != nil {
		return 0, fmt.Errorf("delete airport nodes: %w", err)
	}

	a.mu.Lock()
	kept := make([]*subscription.Node, 0, len(a.nodes))
	...
	a.logger.Info("airport nodes purged", "removed", removed, "kept_self_hosted", len(kept))
