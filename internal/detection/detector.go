...
// nodeLatencyURL 经节点测延迟的小请求端点(Cloudflare 1KB)。
const nodeLatencyURL = "https://speed.cloudflare.com/__down?bytes=1000"

// TestNodeLatency 经节点(含直连出口)打 Cloudflare 小请求 samples 次,算
// latency(min RTT,毫秒)与 jitter(相邻 RTT 差绝对值平均,毫秒)。
//
// 复用 newProxyAdapter 注入直连出口 dialer(TUN 环境下让节点连真实 IP 而非
// fake-ip,否则经节点访问 Cloudflare 会 EOF)。设 bandwidthUserAgent 规避
// Cloudflare 对默认 Go UA 的 403。供本机实测 SSE 的 latency 帧使用。
func (d *Detector) TestNodeLatency(ctx context.Context, node *subscription.Node, samples int) (latencyMs int, jitterMs float64, err error) {
	if samples <= 0 {
		samples = 8
	}
	adapter, err := d.newProxyAdapter(node)
	if err != nil {
		return 0, 0, fmt.Errorf("create proxy adapter: %w", err)
	}
	client := &http.Client{
	...
	for i := 0; i < samples; i++ {
		start := time.Now()
		req, err := http.NewRequestWithContext(ctx, http.MethodGet, nodeLatencyURL, nil)
		if err != nil {
			return 0, 0, fmt.Errorf("create latency request: %w", err)
		}
		req.Header.Set("User-Agent", bandwidthUserAgent)

		resp, err := client.Do(req)
		if err != nil {
			return 0, 0, fmt.Errorf("latency probe %d: %w", i, err)
		}
		io.Copy(io.Discard, io.LimitReader(resp.Body, 4096))
		resp.Body.Close()
		if resp.StatusCode != http.StatusOK {
			return 0, 0, fmt.Errorf("latency probe %d: HTTP %d", i, resp.StatusCode)
		}
		rtts = append(rtts, float64(time.Since(start).Milliseconds()))
	}

	latencyMs, jitterMs = computeRTTMetrics(rtts)
	return latencyMs, jitterMs, nil
}

// computeRTTMetrics 从 RTT 样本算 latency(min)与 jitter(相邻差绝对值平均)。
// 单样本 jitter=0,空样本全 0(避免 NaN)。
func computeRTTMetrics(rtts []float64) (latencyMs int, jitterMs float64) {
	if len(rtts) == 0 {
		return 0, 0
	}
	min := rtts[0]
	...
	jitterMs = sum / float64(len(rtts)-1)
