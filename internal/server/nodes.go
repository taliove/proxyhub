...
// handleCreateSelfNodeFromPool 把请求者合并池中的单个机场节点一键转为自建节点
// (spec #70 / issue #81)。转换在服务端完成,连接参数(含 reality/grpc)不经前端往返。
// 节点不在请求者自己池内 → 404(多租户:不暴露存在性,凭证不出池);
// 身份撞车 → 409(复用 023 唯一约束与既有消息,issue #53)。
func (s *Server) handleCreateSelfNodeFromPool(w http.ResponseWriter, r *http.Request) {
	scope, ok := s.mustUserScope(w, r)
	if !ok {
		return
	}
	...
	n := selfHostedFromPoolNode(src)
	// 身份查重(issue #53):同 server/port/protocol 拒绝重复创建,与手动创建同语义。
	if dup, dErr := s.st.SelfHostedNodeIdentityExists(effUID, n.Server, n.Port, n.Protocol, 0); dErr != nil {
		s.logger.Error("check self node identity failed", "error", dErr)
		http.Error(w, "internal error", http.StatusInternalServerError)
		return
	} else if dup {
	...
	n.Enabled = true
	// 区域:池节点已带聚合期解析的地区码,优先沿用;缺失时走既有解析链(失败不阻断保存)。
	if src.Region != "" {
		n.RegionCode = src.Region
	} else {
		n.RegionCode = s.resolveSelfNodeRegion(n)
	}
	if err := applySelfNodeNameFallback(n); err != nil {
	...
	if err := s.st.CreateSelfHostedNodeForUser(effUID, n); err != nil {
		// check-then-insert 竞态的 DB 兜底(023 唯一约束):与前置查重同款 409
		if errors.Is(err, store.ErrDuplicateIdentity) {
			http.Error(w, "该节点已存在(相同服务器/端口/协议)", http.StatusConflict)
			return
		}
		s.logger.Error("create self node from pool failed", "error", err)
		http.Error(w, "internal error", http.StatusInternalServerError)
		return
	}
	regionResolved := n.RegionCode != "" && n.RegionCode != "Unknown"
...
// selfHostedFromPoolNode 把池节点全量映射为自建节点草稿(spec #70):
// 连接参数(含 VLESS Reality 与 gRPC)逐项拷贝,缺省 network 归一为 tcp
// (与自建节点历史约定一致);健康/元数据字段不带入(由检测链重建)。
func selfHostedFromPoolNode(src *subscription.Node) *store.SelfHostedNode {
	network := src.Network
	if network == "" {
		network = "tcp"
	}
	return &store.SelfHostedNode{
		Name:              src.Name,
		Protocol:          src.Type,
		Server:            src.Server,
		Port:              src.Port,
		...
		SNI:               src.SNI,
		Flow:              src.Flow,
		RealityPublicKey:  src.RealityPublicKey,
		RealityShortID:    src.RealityShortID,
		ClientFingerprint: src.ClientFingerprint,
		GrpcServiceName:   src.GrpcServiceName,
		GrpcAuthority:     src.GrpcAuthority,
	}
}
