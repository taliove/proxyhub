...
```bash
go build ./... && go vet ./...
go test ./...   # 至少跑受影响包;全量更好
```

改了前端:`cd web && npm run build` 先行(go:embed,不构建不生效)。
改了 `scripts/install/`:六个 `test_*.sh` 套件全绿。
既有失败(2 个模板测试 + TestHandleTestNode_MissingTarget)不算回归,其余必须全绿。
